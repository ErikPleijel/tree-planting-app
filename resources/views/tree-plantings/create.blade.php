    ...
    <div class="max-w-xl mx-auto mt-10 p-6 bg-white shadow-md rounded-lg">
        ...
        <form method="POST" action="{{ route('tree-plantings.store') }}" class="space-y-4">
            ...
            <div>
                ...
                <select name="biochar" class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-primary focus:border-primary">
                    <option value="" @selected(old('biochar') === null || old('biochar') === '')>— None / Not recorded —</option>
                    <option value="0.25" @selected(old('biochar') == '0.25')>Minimal — 0.25 kg dry (~1 Liter)</option>
                    <option value="0.50" @selected(old('biochar') == '0.50')>Small tree — 0.5 kg dry (~2 Liters)</option>
                    <option value="1.00" @selected(old('biochar') == '1.00')>Medium tree — 1.0 kg dry (~4 Liters)</option>
                    <option value="2.00" @selected(old('biochar') == '2.00')>Large tree — 2.0 kg dry (~8 Liters)</option>
                </select>
            </div>
